...
class LeadController extends Controller
{
    ...
    public function get(): JsonResponse
    {
        if (! $this->sensitiveData->canView()) {
            request()->query->set(
                'searchFields',
                'title:like;status:=;user_id:=;person_id:=;lead_type_id:=;lead_pipeline_id:=;lead_pipeline_stage_id:='
            );
            request()->query->set('orderBy', 'updated_at');
            request()->query->set('sortedBy', 'desc');
        }
    ...
    public function update(LeadForm $request, int $id): RedirectResponse|JsonResponse
    {
        ...
        $data = $this->sensitiveData->sanitizeInput('leads', $request->all());
    ...
    public function updateStage(int $id)
    {
        ...
        $payload = $this->sensitiveData->sanitizeInput('leads', $payload);
    ...
    public function kanbanLookup()
    {
        ...
        abort_unless(
            $column['filterable'] && isset($column['filterable_options']['repository']),
            404
        );
    ...
    {
        $canViewSensitiveData = $this->sensitiveData->canView();
        ...
        return [
            ...
            [
                ...
                'visibility' => $canViewSensitiveData,
            ],
            [
                'index' => 'user_id',
            ...
            [
                ...
                'filterable' => $canViewSensitiveData,
                'filterable_type' => 'dropdown',
                'filterable_options' => $canViewSensitiveData
                    ? $this->sourceRepository->all(['name as label', 'id as value'])->toArray()
                    : [],
                'allow_multiple_values' => true,
                'sortable' => $canViewSensitiveData,
                'visibility' => $canViewSensitiveData,
    ...
    public function createByAI()
    {
        ...
        return response()->json([
            'message' => trans('admin::app.leads.create-success'),
            'leads' => LeadResource::collection($this->createLeads($leadData)),
        ]);
    }
