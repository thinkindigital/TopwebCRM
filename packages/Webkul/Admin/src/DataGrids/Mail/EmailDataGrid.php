...
class EmailDataGrid extends DataGrid
{
    ...
    public function prepareQueryBuilder(): Builder
    {
        $queryBuilder = DB::table('emails')
            ->select(
                'emails.id',
                DB::raw('COALESCE('.DB::getTablePrefix().'emails.name, '.DB::getTablePrefix().'emails.from) as name'),
                'emails.subject',
                'emails.reply',
                'emails.is_read',
                'emails.created_at',
    ...
    public function prepareColumns(): void
    {
        ...
        $this->addColumn([
            ...
            'searchable' => $canViewSensitiveData,
            'filterable' => $canViewSensitiveData,
            'exportable' => $canViewSensitiveData,
            'closure' => fn ($row) => $canViewSensitiveData
                ? trim($row->name, '"')
        ...
        $this->addColumn([
            ...
            'filterable' => $canViewSensitiveData,
            'exportable' => $canViewSensitiveData,
            'closure' => fn ($row) => $sensitiveData->display('emails', 'subject', $row->subject),
        ]);
        ...
        $this->addColumn([
            ...
            'filterable' => $canViewSensitiveData,
            'exportable' => $canViewSensitiveData,
            'closure' => fn ($row) => $sensitiveData->display('emails', 'reply', $row->reply),
        ]);
