...
@pushOnce('scripts')
    ...
    >
        @if (bouncer()->hasPermission('quotes') && app(\App\Services\SensitiveDataService::class)->canView())
            <div class="p-3">
        ...
        @else
            <p class="p-4 text-gray-600 dark:text-gray-300">{{ config('sensitive-data.mask') }}</p>
        @endif
    </script>
    ...
    <script type="module">
        app.component('v-lead-quotes', {
            ...
            data: function () {
                return {
                    quotes: @json(app(\App\Services\SensitiveDataService::class)->canView()
                        ? $lead->quotes()->with(['person', 'user'])->get()
                        : []),
                }
            },
